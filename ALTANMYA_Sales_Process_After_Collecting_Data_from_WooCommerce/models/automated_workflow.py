        ...
        try:
            ...
            for rec1 in self.ids:
                ...
                if rec.state == 'sale' and rec.woo_status=='completed':
                    ...
                    rec.invoice_ids.invoice_date=rec_date_order
...
class extend_connector(models.Model):
    _inherit = ["woo.commerce.ept"]
    journal_id =fields.Many2one('account.journal',string='Journal',domain="[('type', 'in', ('bank','cash'))]")


class extend_woo_sale_order(models.Model):
    _inherit = ["sale.order"]
    payment_gateway_id=fields.Many2one('woo.commerce.ept')
    woo_status =fields.Selection([('completed', 'Completed'), ('cancelled', 'Cancelled')], string='WooCommerce status')
